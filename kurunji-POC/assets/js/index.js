    ...
    if (typeof fetchPublicFeedbacks === 'function') {
        try {
            ...
            if (data && data.feedbacks && data.feedbacks.length > 0) {
                ...
                if (reviewsGrid) {
                    reviewsGrid.innerHTML = ''; // clear static reviews
                    ...
                    let displayReviews = data.feedbacks.slice(0, 3);
                    ...
                    displayReviews.forEach(r => {
                        ...
                        card.className = "bg-white p-8 rounded-3xl shadow-md border border-outline-variant/10 hover:-translate-y-2 hover:shadow-xl transition-all duration-300";
                        card.innerHTML = `
